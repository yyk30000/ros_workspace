...
#define OFFSET_X 302459.942
#define OFFSET_Y 4122635.537
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher path_pub = p.advertise<sensor_msgs::PointCloud>("/Path", 1000);

  ros::Rate loop_rate(1);

  string file_path=ros::package::getPath("path_pub");
...
for(int i=1;i<352;i++)
{
  ...
  read_txt(result,&first);
    ...
    for(int i=1;i<7;i++)
    {
      string a =file_path+"/path_pub/kcity_PM0138/ParkingLot_UTM52N_";
      string b;
      string c;
      string d=".csv";
...
/*
    for(int i=1;i<466;i++)
    {
      string a ="/home/a/Localization/GPS_imu_ukf_Ws/src/path_pub/kcity_PM0138/A3LINK_PathCenter_";
      string b;
      string c;
      string d=".csv";
...
while (p.ok()){
    ...
    path_pub.publish(first);    //path pub

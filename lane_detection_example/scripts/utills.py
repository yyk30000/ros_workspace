...
class STOPLineEstimator:
    ...
    def estimate_dist(self, percentile):
...
class CURVEFFit:
    ...
    def preprocess_pts(self, lane_pts):
        ...
        y_left =y_g[np.logical_and(y_g>=y_ransac_collect_r-self.y_margin, y_g<y_ransac_collect_r+self.y_margin)]
        x_left =x_g[np.logical_and(y_g>=y_ransac_collect_r-self.y_margin, y_g<y_ransac_collect_r+self.y_margin)]
    ...
    def fit_curve(self, lane_pts):
        ...
        X_pred =np.stack([x_pred**1 for i in reversed(range(1, self.order+1))]).T
...
class BEVTransform:
    ...
    def build_tf(self, params_cam):
        ...
        xc,yc,zc=xyz_bird[0,:].reshape([1,-1]), xyz_bird[1,:].reshape([1,-2]), xyz_bird[2,:].reshape([1,-1])
    ...
    def project_lane2img(self, x_pred,y_pred_l, y_pred_r):
        ...
        xyl =self.project_ptsimg(xyz_l_b)
        xyr =self.project_ptsimg(xyz_r_b)

        xyl=self.crop_pts(xyl)
        xyr=self.crop_pts(xyr)
        
        return xyl, xyr
    def project_ptsimg(self, xyz_brid):
        xc, yc,zc =xyz_brid[0,:].reshape([1,-1]), xyz_brid[1,:].reshape([1,-1]), xyz_brid[2,:].reshape([1,-1])
...
def warp_image(img,source_prop):
    ...
    perspective_transform=cv2.getPerspectiveTransform(source_points,destination_point)

...
class path_pub_tf :
    ...
    def __init__(self):
        ...
        vel_planner=velocityPlanning(50,0.15)
        ...
        while not rospy.is_shutdown():
            if self.is_imu == True:
                ...
                if self.is_odom == True :
                    ...
                    vel_msg=Float64()
                    vel_msg.data=self.acc.acc(loi,self.speed,vel_profile[current_waypoint])
                    # print(vel_profile[current_waypoint])
                    # print("")
                    # print(loi)
                    # print("")
                    # print(self.ego_pose)
                self.acc.checkObject(local_path_msg,goi,loi)
                print("1")
                print(self.acc.acc(loi,self.speed,vel_profile[current_waypoint]))
                print("2")
                print(vel_profile[current_waypoint])
    ...
    def core_callback(self,msg):
        self.is_speed=True
        rpm=msg.state.speed
        self.speed=rpm/self.rpm_gain *3.6
        # print("speed")
        # print(self.speed)
    ...
    def calc_vaild_obj(self,ego_pose):
        ...
        if self.all_object.num_of_objects > 0:
            ...
            for num in range(len(self.vaild_stoplane_position)):
                ...
                if local_result[0][0]> 0:
                    global_obect_info.append([1,global_result[0][0],global_result[1][0],0])

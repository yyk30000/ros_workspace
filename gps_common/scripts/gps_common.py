...
class LocationSensor:
    def __init__(self, zone=52):
        ...
        self.PO = 0 / self.DO
        self.LO = (6 * abs(self.zone)-183) / self.DO
        ...
        self.YS = self.YO - self.N *(
            self.C[0]* self.PO
            + self.C[1] * np.sin(2 * self.PO)
            + self.C[2] * np.sin(4 * self.PO)
            + self.C[3] * np.sin(6 * self.PO)
            + self.C[4] * np.sin(8 * self.PO))  
    ...
    def navsat_callback(self, gps_msg): 
        ...
        e_global , b_global = e_o, n_global = n_o
...
def navsat_callback(self, gps_msg):
    ...
    e_global = self.convertLL2UTM(lat.lon)

    x,y = e_global - e_o, n_global - n_o

    odom_msg = Odometry()
    odom_msg.child_frame_id='base link'
    odom_msg.header.frame_id='map'
    odom_msg.header.stamp=rospy.Time.now()
    odom_msg.header.stamp = rospy.Time.now()
    odom_msg.pose.pose.position.x=x        
